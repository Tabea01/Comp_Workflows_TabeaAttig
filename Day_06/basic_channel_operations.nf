...
workflow{
    ...
    //           Write the output of each channel to a list
    ...
    if (params.step == 13) {
        ...
        // Filter even and odd numbers into separate channels
    ...
    if (params.step == 14) {
        ...
        // Collect names and write to a file
        in_ch.map { it.name }
            .toList()
            .subscribe { names ->
                // Write names to results/names.txt
                new File('results/names.txt').withPrintWriter { writer ->
                    names.each { name ->
                        writer.println(name)
                    }
                }
